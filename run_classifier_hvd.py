...
def get_model_fn(n_class):
  def model_fn(features, labels, mode, params):
    ...
    scaffold_fn = model_utils_hvd.init_from_checkpoint(FLAGS)
    ...
    train_op, learning_rate, _ = model_utils_hvd.get_train_op(FLAGS, total_loss)
...
def main(_):
  ...
  FLAGS.output_dir = FLAGS.output_dir if hvd.rank() == 0 \
    else os.path.join(FLAGS.output_dir, str(hvd.rank()))
  #FLAGS.output_dir = os.path.join(FLAGS.output_dir, str(hvd.rank()))
  ...
  run_config = model_utils_hvd.configure_tpu(FLAGS)
  ...
  if FLAGS.do_train:
    ...
    num_train_steps = FLAGS.train_steps // hvd.size()

    hvd_hooks = [hvd.BroadcastGlobalVariablesHook(0)]
    estimator.train(
        input_fn=train_input_fn, max_steps=num_train_steps, hooks=hvd_hooks)
